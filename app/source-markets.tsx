...
function Quote({quote:q,type}:{quote:HrDisplayQuote;type:number}){
 return <div className="space-y-2 border-t border-white/10 py-3 text-sm">
  ...
  {!q.open?<p className="text-slate-400">暫不顯示盤口與賠率</p>:type===104||type===105?<>
   ...
   {(type===103||type===106)&&<p className="text-base font-bold">{q.homeLine!==''?`主讓 ${q.homeLine}`:q.awayLine!==''?`客讓 ${q.awayLine}`:'盤口未提供'}</p>}
