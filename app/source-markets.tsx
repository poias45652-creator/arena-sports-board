...
const types=[{id:103,label:'讓球'},{id:104,label:'大小'},{id:111,label:'獨贏'},{id:106,label:'一輸'},{id:105,label:'單雙'}];
...
function Quote({quote:q,type}:{quote:HrDisplayQuote;type:number}){
 return <div className="space-y-2 border-t border-white/10 py-3 text-sm">
  ...
  {!q.open?<p className="text-slate-400">暫不顯示資料與賠率</p>:type===104||type===105?<>
   {type===104&&<p className="text-base font-bold">大小 {text(q.total)}</p>}
   <p className="tabular-nums">{type===105?'單':'大'} @{text(q.over)} ／ {type===105?'雙':'小'} @{text(q.under)}</p>
  </>:<>
   {(type===103||type===106)&&<p className="text-base font-bold">{q.homeLine!==''?`主讓 ${q.homeLine}`:q.awayLine!==''?`客讓 ${q.awayLine}`:'資料未提供'}</p>}
...
export default function SourceMarkets({data,error,loading,refresh,now,day}:{data:SuperSnapshot|null;error:string;loading:boolean;refresh:()=>Promise<void>;now:number;day:string}){
 ...
 return <section className="panel space-y-4 p-4 sm:p-5" aria-label="MLB 原始數值展示">
  <div className="flex flex-wrap items-center justify-between gap-3">
   <div><h3 className="text-xl font-black">MLB 資料</h3><p className="mt-1 text-sm text-slate-400">{day}（台灣）・{data?.source==='hr9988'?'SUPER':data?.source||'SUPER'}・賽前盤</p></div>
   <Button variant="outline" disabled={loading} onClick={()=>void refresh()}>{loading?'讀取中…':'更新資料'}</Button>
  </div>
  <Tabs value={period} onValueChange={setPeriod}>
   <TabsList aria-label="賽事時段"><TabsTrigger value="full">全場</TabsTrigger><TabsTrigger value="firstHalf">上半場</TabsTrigger></TabsList>
   <p className="my-3 text-sm text-slate-400">來源原始數值與賠率・僅供查看。上半場範圍及結算依來源規則。</p>
   <p className="mb-4 text-sm text-slate-400">{data?.fetchedAt?`最後取得：${new Date(data.fetchedAt).toLocaleString('zh-TW',{timeZone:'Asia/Taipei'})}（台灣）`:'尚未取得資料'}・網頁開啟時每分鐘更新</p>
   {periods.map(p=><TabsContent key={p.id} value={p.id} className="space-y-4">
    {!valid?<p role="status" className="text-sm text-amber-200">{error||(loading?'正在讀取你的會員資料…':'資料尚未取得或已過期，請更新資料。')}</p>:!games.length?<p role="status" className="text-sm text-slate-400">來源目前沒有此日期的 MLB 未開賽場次。</p>:games.map(g=>{
     ...
     return <article key={g.id} className="rounded-lg border border-white/10 p-4">
      ...
      {!g.displayMarkets?<p className="mt-3 text-sm text-amber-200">等待新版資料，請稍後更新。</p>:<div className="mt-4 grid gap-4 md:grid-cols-3">
       {types.filter(t=>p.id==='full'?t.id!==105:[103,104,105].includes(t.id)).map(t=>{const rows=markets.filter(m=>m.period===p.id&&m.type===t.id).flatMap(m=>m.quotes);const primary=rows.filter(q=>q.primary),others=rows.filter(q=>!q.primary);
