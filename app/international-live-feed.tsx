...
type Feed={league:string;date:string;games:Game[];status:string;stale:boolean;noGames?:boolean;nextGameDate?:string;checkedAt?:string;error?:string;pollAfterMs?:number;persistence?:{error?:string}};
...
export default function InternationalLiveFeed({league,revision,onGameDetails}:{league:'NPB'|'KBO'|'CPBL';revision:number;onGameDetails?:(id:string)=>void}){
 ...
 return <section className="live-center" aria-label={`${league} 即時比分`}>
  ...
  <div className="live-center-filters"><div className="score-filters" role="group" aria-label="篩選比分狀態">{[['all','全部'],['live','進行中'],['upcoming','未開賽'],['final','已完賽']].map(([value,label])=><Button key={value} variant="ghost" aria-pressed={filter===value} onClick={()=>setFilter(value)}>{label}</Button>)}</div><p>{data?.games.length?`更新 ${stamp(data.games[0]?.source.fetchedAt,true)}（台灣）`:data?.noGames?`已核對 ${stamp(data.checkedAt,true)}（台灣）`:loading?'正在同步…':'等待來源更新'}</p></div>
