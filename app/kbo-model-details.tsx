...
export default function KboModelDetails({game}:{game:PregameGame}){
 ...
 return <details className="rounded-lg border border-slate-600 bg-slate-900/50">
  ...
  <div className="space-y-3 px-4 pb-4 text-sm text-slate-300">
   ...
   <p>前九局每局得分率＝（對方先發 ERA × {RUN_MODEL_WEIGHTS.starter*100}% ＋ 己方得分能力 × {RUN_MODEL_WEIGHTS.offense*100}% ＋ 對方團隊失分 × {RUN_MODEL_WEIGHTS.defense*100}% ＋ 對方牛棚 ERA × {RUN_MODEL_WEIGHTS.bullpen*100}%）÷ 9。本季至少 20 場；主客場分項至少 10 場，加入 20 場本季均值收縮。先發 ERA 加入 20 局、牛棚 ERA 加入 60 局團隊失分均值收縮；權重與收縮量均為初始設定。</p>
   ...
   <p>先發參考局數採近 60 日最多 5 場登板平均，至少 3 場、限制 3–7 局；不足時列 5 局假設，僅供後台參考，不改變固定權重。牛棚 ERA 與局數為必要欄位，不以排行榜或空值代填。ERA 與總失分差異、WHIP、傷停、確認打線、天氣、球場及近期牛棚用量未另建模。</p>
   ...
   <p>獨贏和局退回；讓分、大小、中洞比例按來源報價結算。全贏、中洞贏、中洞輸、全輸，加走盤共 100%。單場推薦參考預期收益；自動串關與美棒相同，按全贏加中洞贏機率排序，須高於輸的機率，每場一項、同隊不重複。</p>
