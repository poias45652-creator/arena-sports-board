...
export default function InternationalMarketAnalysis({league,game,market,onMarketChange,picks,onPick,canPick,status,analysis=null,quotesFresh=false}:{
 ...
 const preferred=[...outcomes].filter(o=>o.expectedProfit>0).sort((a,b)=>b.expectedProfit-a.expectedProfit)[0]?.pick.side;
       ...
       return <Button key={side} variant={active?'default':'outline'} disabled={!canPick||!pick} aria-pressed={active}
        ...
        <span className="block w-full">
         <span className="market-pick-title flex w-full flex-wrap items-start justify-between gap-x-3 gap-y-1 font-bold">
          <span className="inline-flex min-w-0 flex-wrap items-center gap-2">{!total&&!oddEven&&<InternationalTeamLogo league={league} name={game[side]} size={24}/>}<span>{title}</span>{preferred===side&&<span className="text-sm text-emerald-400">{analysis?.mode==='simulation'?'模擬推薦':'推薦'}</span>}{active&&<span aria-label="已選取">✓</span>}</span>
