...
async function collect(){
 ...
 if(!r.ok)throw new SourceError('source_unavailable','Super007 暫時無法提供資料，稍後自動重試。');
 ...
 if(!Array.isArray(d.data?.List))throw new SourceError('source_format_changed','Super007 資料格式改變，暫停推薦。');
...
export async function GET(){
 ...
 try{
  ...
  if(d.source!=='Super007'||!Array.isArray(d.games)||!Number.isFinite(age)||age< -60000||age>150000)throw new Error('雲端資料已過期，暫停推薦');
