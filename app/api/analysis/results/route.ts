...
export async function POST(request:Request){
 ...
 try{
  ...
  const paired=await db.prepare('SELECT s.payload,r.away,r.home FROM analysis_snapshots s JOIN analysis_results r ON r.game_id=s.game_id WHERE s.member_id=? AND s.captured_at<s.start_time AND s.start_time=r.scheduled_start AND s.captured_at=(SELECT MAX(s2.captured_at) FROM analysis_snapshots s2 WHERE s2.member_id=s.member_id AND s2.game_id=s.game_id AND s2.captured_at<s2.start_time) ORDER BY s.captured_at DESC LIMIT 2000').bind(memberId).all();
  ...
  const recorded=await db.prepare(`SELECT s.payload AS payload FROM analysis_snapshots s WHERE s.member_id=? AND s.captured_at<s.start_time AND s.captured_at=(SELECT MAX(s2.captured_at) FROM analysis_snapshots s2 WHERE s2.member_id=s.member_id AND s2.game_id=s.game_id AND s2.captured_at<s2.start_time) ORDER BY s.captured_at DESC LIMIT 500`).bind(memberId).all();
  ...
  const count=await db.prepare('SELECT COUNT(*) AS snapshots,COUNT(DISTINCT game_id) AS games FROM analysis_snapshots WHERE member_id=?').bind(memberId).first();
