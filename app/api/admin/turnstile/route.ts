...
export async function GET(){
 ...
 try{const config=await readTurnstileSettings(getRawDb(),env.TZ_BINDING_KEY);return reply({enabled:config.enabled});}catch{return reply({error:'設定讀取失敗，請稍後重試'},503);}
}
