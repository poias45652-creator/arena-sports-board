...
import {saveTurnstileSettings} from '@/lib/turnstile-settings';
export const dynamic='force-dynamic';
const reply=(data:unknown,status=200)=>Response.json(data,{status,headers:{'Cache-Control':'private, no-store'}});
export async function GET(){
 if(!await isSiteAdmin())return reply({error:'僅限管理員'},403);
 try{const row=await getRawDb().prepare("SELECT enabled FROM turnstile_settings WHERE id='login'").first();return reply({enabled:!!row?.enabled});}catch{return reply({error:'設定讀取失敗，請稍後重試'},503);}
}
export async function POST(request:Request){
 ...
 try{
  ...
  if(input.enabled===false){await getRawDb().prepare("UPDATE turnstile_settings SET enabled=0,updated_at=? WHERE id='login'").bind(Date.now()).run();return reply({enabled:false});}
  ...
  await saveTurnstileSettings(getRawDb(),key,secret);return reply({enabled:true});
