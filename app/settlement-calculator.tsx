...
export default function SettlementCalculator(){
 ...
 return <details className="panel p-5"><summary className="cursor-pointer font-bold">串關結算試算</summary><p className="my-3 text-sm text-slate-400">依 Super007 資料對照與棒球過關範例計算。輸入已確認有效的賽果；試算不會送出投注。讓分欄以主隊讓分為基準，賠率不含本金。</p>
 ...
 <label className="text-sm">原始數值<Input value={line} disabled={!['spread','total'].includes(market)} onChange={e=>setLine(e.target.value)} placeholder="1+75、7-55、0.5/1"/></label>
 ...
 </div><p className="my-3" role="status">{fraction===null?'請輸入有效數值、分數與金額。':`${fraction===0?'走盤／退回':fraction>0?'贏 '+(fraction*100).toFixed(0)+'%':'輸 '+(-fraction*100).toFixed(0)+'%'} · 淨損益 ${result!.toFixed(2)} · 含本金返還 ${(amount+result!).toFixed(2)}`}</p>
 <Button variant="outline" disabled={fraction===null||legs.length>=10} onClick={()=>fraction!==null&&setLegs([...legs,{fraction,price:odds}])}>加入串關試算</Button><p className="my-3 text-sm text-slate-400">每次加入須為不同賽事；此處是手動情境，不代表已選的推薦單。取消或走盤按倍數 1 計，中洞輸保留剩餘本金。棒球過關最高可贏額依官方規則為 200 萬，以下先顯示未套限額的公式結果。</p>
