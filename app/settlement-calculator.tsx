...
export default function SettlementCalculator(){
 ...
 </div><p className="my-3" role="status">{fraction===null?'請輸入有效盤口、分數與金額。':`${fraction===0?'走盤／退回':fraction>0?'贏 '+(fraction*100).toFixed(0)+'%':'輸 '+(-fraction*100).toFixed(0)+'%'} · 淨損益 ${result!.toFixed(2)} · 含本金返還 ${(amount+result!).toFixed(2)}`}</p>
