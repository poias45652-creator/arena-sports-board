...
function ParlayPane({children}:{children:ReactNode}){
...
export default function Markets({analysis,games,now,data,error,scheduleOK,odds,oddsError,oddsLoading,refreshOdds,renderMatchHeader,renderWinnerOptions,winnerPanel,parlayMode,onParlayModeChange}:{analysis:Record<number,AnalysisState>;games:Match[];now:number;data:RunSnapshot|null;error:string;scheduleOK:boolean;odds:OddsSnapshot|null;oddsError:string;oddsLoading:boolean;refreshOdds:()=>Promise<void>;renderMatchHeader:(game:Match,expectedRunsInfo:ReactNode)=>ReactNode;renderWinnerOptions:(game:Match)=>ReactNode;winnerPanel:ReactNode;parlayMode:'markets'|'winner';onParlayModeChange:(mode:'markets'|'winner')=>void}){
 ...
 function reason(g:Match|undefined){
  return !g?'已切換日期或賽程已移除':!scheduleOK?'賽程更新中或已過期':!isPregame(g,now)?'已開賽或賽事狀態改變':!dataOK?'得失分資料未齊或已過期':marketContextStatus(g,analysis[g.id]?.report,now).blocked||(automatic&&!oddsOK?'盤口讀取失敗或已過期，暫停推薦':automatic&&!matchOdds(g,odds)?'尚未開盤':!models.get(g.id)?'本季得失分樣本不足':'');
 }
 ...
 function update(g:Match,field:'spread'|'total',value:string){
  ...
  if(picks.some(p=>p.gameId===g.id&&p.key===field)){setPicks(picks.filter(p=>!(p.gameId===g.id&&p.key===field)));setNotice('盤口已更改，該關已移除，請重新確認方向。');}
 }
 function recommend(){
 ...
 function marketPanel(g:Match,key:BoardPick['key']){
  ...
  return <section className="space-y-3" aria-label={boardMarketLabel(key)}>
   ...
   {(key==='spread'||key==='total')&&!automatic?<><Input aria-label={boardMarketLabel(key)+'自訂盤口'} type="number" step="0.25" min={key==='spread'?-10:.5} max={key==='spread'?10:30} value={manualLines(g)[key]} onChange={e=>update(g,key,e.target.value)}/><p className="text-sm text-slate-400">自訂參考線 請核對實際盤口</p></>:q&&key!=='firstHalfOddEven'?<p className="market-line">{key==='total'||key==='firstHalfTotal'?q.display??q.line:formatSpreadLine(q,'home')}</p>:null}
   ...
   {!q&&<p className="text-sm text-amber-200">{automatic?(oddsError||(sourceIssue?.startsWith('來源沒有開放的主盤')?'尚未開盤':sourceIssue)||'尚未開盤'):'請輸入有效的自訂盤口。'}</p>}
   {blocked&&q&&<p className="text-sm text-amber-200">{blocked}</p>}
  </section>;
 }
 return <section className="arena-analysis-board" aria-label="每日對戰勝率與盤口">
  <div className="arena-analysis-controls space-y-4">
  <h3 className="text-xl font-black">每日對戰 勝率與盤口</h3>
  <div className="flex flex-wrap items-center gap-3"><Select value={mode} onValueChange={v=>{setMode(v);setPicks([]);setMarketTabs({});setNotice('已切換模式，請重新選關。');}}><SelectTrigger aria-label="盤口來源"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="super007">{sourceName} 盤口</SelectItem><SelectItem value="manual">自訂盤口試算</SelectItem></SelectContent></Select><Button variant="outline" disabled={oddsLoading} onClick={()=>void refreshOdds()}>{oddsLoading?'讀取中…':'更新盤口'}</Button><span className="text-sm text-white">抓取：{odds?.fetchedAt?new Date(odds.fetchedAt).toLocaleString('zh-TW',{timeZone:'Asia/Taipei'}):'尚未取得'}（台灣） 來源可能延遲</span></div>
  {automatic&&!oddsOK&&<p role="status" className="text-sm text-amber-200">{oddsError||'盤口尚未取得或已過期，暫停推薦。'} 舊資料不參與推薦。</p>}
  <p className="text-sm text-white">{sourceName} 原始主盤每分鐘讀取；支援全場與上半場七種玩法。全贏、中洞贏、平手退回與中洞輸分開計算。</p>
  {!dataOK&&<p role="status" className="text-sm text-amber-200">{error||'正在取得球隊得失分資料，暫停推薦。'}</p>}
  </div>
  <div className="arena-analysis-summary"><ParlayPane><aside className="panel space-y-4 p-5"><h3 className="text-lg font-black">串關組合</h3><Tabs value={parlayMode} onValueChange={v=>onParlayModeChange(v as 'markets'|'winner')}><TabsList className="mb-3 h-auto min-h-10 w-full" aria-label="選擇串關玩法"><TabsTrigger value="markets">盤口</TabsTrigger><TabsTrigger value="winner">獨贏</TabsTrigger></TabsList><TabsContent value="markets" className="parlay-compact-content space-y-2">
   <div className="parlay-compact-controls">
   <Select value={String(count)} onValueChange={v=>{setCount(Number(v));setPicks(p=>p.slice(0,Number(v)));}}><SelectTrigger aria-label="盤口串關數量" className="w-full"><SelectValue/></SelectTrigger><SelectContent>{[3,4,5].map(n=><SelectItem key={n} value={String(n)}>{n} 關</SelectItem>)}</SelectContent></Select>
  ...
  <div className="arena-analysis-games arena-market-layout space-y-4">
   {!games.length&&<p className="panel p-5 text-slate-400">這個台灣日期沒有賽事，請切換上方日期。</p>}
   {orderedGames.map(g=>{
    ...
    return <article className="panel overflow-hidden" key={g.id} data-game-id={g.id}>
     ...
     {isPregame(g,now)&&<details className="match-market-details"><summary><span>查看盤口與分析</span><span className="text-sm font-normal">{automatic?'7 種玩法':'自訂盤口'}</span></summary><div className="space-y-4 p-4">
      <Tabs value={selected} onValueChange={value=>setMarketTabs(old=>({...old,[g.id]:value as MarketKey}))}>
       <div className="market-tabs-scroll"><TabsList className="market-type-tabs" aria-label="選擇盤口玩法">{BOARD_MARKETS.filter(({key})=>automatic||key==='spread'||key==='total'||key==='moneyline').map(({key,label})=><TabsTrigger key={key} value={key}>{label}</TabsTrigger>)}</TabsList></div>
