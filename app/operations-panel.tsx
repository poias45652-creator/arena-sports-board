...
export default function OperationsPanel({evaluation}:{evaluation:any}){
 ...
 <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr><th>玩法</th><th>全贏</th><th>中洞贏</th><th>走盤</th><th>中洞輸</th><th>全輸</th><th>淨損益（單位）</th></tr></thead><tbody>{(['spread','total'] as const).map(k=>{const m=evaluation?.markets?.[k];return <tr key={k}><td>{k==='spread'?'讓分':'大小'}</td>{['win','partialWin','push','partialLoss','loss'].map(f=><td key={f}>{m?.[f]??0}</td>)}<td>{Number(m?.profitUnits??0).toFixed(2)}</td></tr>;})}</tbody></table></div>
 <div className="space-y-2">{(evaluation?.ledger??[]).slice(0,20).map((r:any,i:number)=><div className="rounded border border-white/10 p-3" key={i}>{r.away} vs {r.home} · {r.score.away}：{r.score.home}<br/>{r.market==='spread'?'讓分':'大小'} · {r.side} · {r.line} @ {r.netOdds} · {r.fraction>0?'贏':r.fraction<0?'輸':'走盤'} {Math.abs(r.fraction*100).toFixed(0)}% · 淨損益 {r.profitUnits.toFixed(3)}<br/><span className="text-slate-400">保存時間：{new Date(r.capturedAt).toLocaleString('zh-TW',{timeZone:'Asia/Taipei'})}</span></div>)}</div>
 </div></details>;
}
