...
export default function WinBreakdown({game,scheduleFresh}:{game:Match;scheduleFresh:boolean}){
 ...
 <details className="mt-4 rounded-lg border border-white/10 p-3 text-sm"><summary className="cursor-pointer font-bold text-amber-200">勝率怎麼算・已採用 {Number(result.coverage.toFixed(2))}% 權重（試算）</summary><p className="my-3 text-slate-300">先發投手合計 60%（本季 ERA 30%、WHIP 15%、近期 ERA 15%）；其餘因素依原比例合計 40%。表格只將顯示值四捨五入，計算使用完整精度。這是未回測校準的多因素試算。從 50% 起算，加上以下主隊影響；客隊為 100% 減主隊。缺項不補值、不重新分配權重。{result.reason}</p><div className="overflow-x-auto"><table className="w-full min-w-[540px] text-left"><thead><tr><th>因素</th><th>設定權重</th><th>客隊原值</th><th>主隊原值</th><th>主隊影響</th></tr></thead><tbody>{result.factors.map(f=><tr key={f.name} className="border-t border-white/10"><td className="py-2" title={f.rule}>{f.name}<small className="block max-w-60 text-slate-400">{f.rule}</small></td><td>{Number(f.weight.toFixed(2))}%</td><td>{f.away?.toFixed(2)??'—'}</td><td>{f.home?.toFixed(2)??'—'}</td><td>{f.score===null?'未採用':`${f.contribution>=0?'+':''}${f.contribution.toFixed(2)} 百分點`}</td></tr>)}</tbody></table></div><p className="mt-3 text-slate-400">各項差值依列示尺度換算，限制在 −1 至 +1；影響＝權重 × 分數 ÷ 2。這些尺度與權重是初始設定，並非統計驗證的因素重要性。賠率不加入球隊勝率；獨贏依較高試算勝率排序，不代表報酬較佳。讓分／大小分仍使用另列的球季得失分模型。</p></details></section>;
}
