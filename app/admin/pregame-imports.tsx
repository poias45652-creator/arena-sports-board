...
export default function PregameImports(){
 ...
 useEffect(()=>{let active=true,busy=false;const c=new AbortController();async function load(){if(busy||document.hidden)return;busy=true;try{await Promise.all(['CPBL','NPB','KBO'].map(async league=>{try{const r=await fetch(`/api/international?kind=${league.toLowerCase()}-pregame`,{cache:'no-store',signal:AbortSignal.any([c.signal,AbortSignal.timeout(45000)])});if(!r.ok)throw Error();const data=await r.json();if(active)setFeeds(old=>({...old,[league]:data}));}catch{if(active)setFeeds(old=>({...old,[league]:{...old[league],status:'stale',error:'更新暫時失敗，稍後自動重試'}}));}}));}finally{busy=false;}}void load();const timer=setInterval(()=>void load(),300000),resume=()=>{if(!document.hidden)void load();};window.addEventListener('arena-refresh-all',resume);document.addEventListener('visibilitychange',resume);return()=>{active=false;c.abort();clearInterval(timer);window.removeEventListener('arena-refresh-all',resume);document.removeEventListener('visibilitychange',resume);}},[]);
 const pregameImports:PregameData[]=initialImports.map(original=>feeds[original.league]?.pregame||original);
 return <section className="panel admin-section" aria-label="賽前資料匯入與缺漏">
  <details><summary className="cursor-pointer p-5 text-lg font-bold">賽前資料、自動更新與缺漏 · {pregameImports.reduce((n,d)=>n+d.games.length,0)} 場</summary>
   <div className="space-y-5 px-4 pb-5">
    <p className="text-sm text-slate-300">網頁開啟時：盤口及場中比分約每 60 秒更新，賽前資料每 5 分鐘检查。中職使用非官網来源；日職使用 Sportsnavi，韓職使用 Naver。更新失敗保留最後成功紀錄，補入的歷史欄位保留原始時間。</p>
    <p className="text-sm text-amber-200">關閉所有網頁後的背景排程尚未啟用；排程接收介面已準備完成，等待指定服務。</p>
    <div className="grid gap-3 md:grid-cols-3">{['CPBL','NPB','KBO'].map(league=>{const d=feeds[league];return <div key={league} className="rounded-lg border border-slate-600 p-3 text-sm"><h3 className="font-bold">{league} · {d?d.status==='ready'?'本次來源已更新':d.status==='partial'?'部分來源已更新':d.status==='stale'?'保留上次資料':'資料待補':'正在檢查'}</h3><p className="mt-2 text-slate-300">檢查時間：{d?.checkedAt?new Date(d.checkedAt).toLocaleString('zh-TW',{timeZone:'Asia/Taipei',hour12:false}):'—'}</p>{d?.live&&<p className="mt-2">比分 {d.live.games} 場 · 先發 {d.live.starters} 位 · 完整打序 {d.live.lineups} 隊</p>}{d?.coverage&&<p>賽前 ERA {d.coverage.era}/{d.coverage.teams} · WHIP {d.coverage.whip}/{d.coverage.teams}</p>}{(d?.error||d?.storageError)&&<p className="mt-2 text-amber-200">{d.error||d.storageError}</p>}{d?.forecastStorage?.error&&<p className="mt-2 text-amber-200">{d.forecastStorage.error}</p>}</div>})}</div>
    ...
    <div className="rounded-lg border border-amber-300/30 bg-amber-300/5 p-4 text-sm"><h3 className="mb-3 font-bold text-amber-200">尚缺資料與功能</h3><ul className="list-disc space-y-2 pl-5">
     ...
     <li>中職：已接 Yahoo 非官網逐場日誌，自行彙算團隊得失分、先發與後援成績；投手表涵蓋率見下方。仍缺完整投球數與未公布的先發，新聞先發只適用指定日期。</li>
    ...
    {pregameImports.map(data=>{const s=summarizePregameImport(data);return <details key={data.league} className="rounded-lg border border-slate-600"><summary className="cursor-pointer p-4 font-bold">{data.league} · {data.date} · {s.games} 場資料明細</summary><div className="space-y-3 px-4 pb-4">
     <p className="text-sm text-slate-300">擷取 {new Date(data.observedAt).toLocaleString('zh-TW',{timeZone:'Asia/Taipei',hour12:false})}（台灣）</p>
