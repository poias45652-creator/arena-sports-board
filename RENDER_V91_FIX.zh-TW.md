...
- 登出失敗時保留頁面並顯示可重試的訊息。
- 保留登入與首次管理員設定頁；登入頁網址為 `/login`。會員帳號依原版設定由管理員建立，前台不開放自行註冊。
...
前一包只修復入口，漏掉原本的會員建立功能與登入畫面。本包從已驗證校驗碼的第 74 版 Render 封存程式，取回這些指定檔案，並套用到目前第 91 版程式：

- `app/admin/tools.tsx`：恢復「新增會員帳號」表單（會員帳號、密碼、新增帳號），保留目前的投打對照、更新狀態與結算工具。
- `app/api/meta/route.ts`：恢復管理員新增會員 API，驗證登入角色、同站來源與輸入格式；密碼經雜湊儲存，新帳號固定為會員，管理員自己的登入狀態不變。
- `server/auth.mjs`：恢復禁止公開註冊的原設定，會員須由管理員建立。
- `app/login/page.tsx`、`form.tsx`、`login.css`：恢復 YJ 圖示、「YJ體育分析」與「登入 YJ體育分析」，欄位為「帳號」「密碼」，保留首次管理員設定。
- `public/0912-bg.mp4`：包含原本 30 秒、1920 × 1080 影片，登入頁靜音自動循環播放；本包不再只包含無法直接播放的 base64 檔。
- `tests/render-auth.test.mjs`：恢復對應的會員建立與公開註冊禁止測試。

保留前一包的管理後台／登出入口、文字調整與第 91 版主要網站內容，不重建既有帳號或資料庫。

本次通過正式建置及 TypeScript；本機正式伺服器以隔離模擬資料庫完成管理員建立會員、新會員密碼登入、重複帳號阻擋、一般會員與跨站請求阻擋、禁止公開註冊及管理員登入狀態保留檢查。亦確認登入頁品牌文字及影片的 HTTP 分段讀取。這些檢查不使用線上帳號或資料庫。
