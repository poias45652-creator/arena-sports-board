...
export async function handleTzBinding(request:Request, memberId:string|null, getDatabase:()=>Database, secret:string|undefined, fetcher:typeof fetch=fetch):Promise<Response>{
 if(!memberId)return fail('signin_required','請先登入 Arena，再綁定自己的 tz 帳號。',401);
 ...
 try{
  ...
  if(!reader)return fail('invalid_request','請輸入 tz 帳號與密碼。',400);
  ...
  if(request.method==='PATCH'){
   ...
   if(!saved)return fail('binding_required','請先完成有效的 tz 帳號綁定，再設定 SUPER 網址。',409);
   return reply({...summary(saved),configured:true,message:'SUPER 網址已儲存。盤口連線尚未驗證。'});
  }
  if(!input||typeof input.username!=='string'||typeof input.password!=='string'||!input.username.trim()||input.username.length>128||!input.password||input.password.length>256)return fail('invalid_request','請輸入有效的 tz 帳號與密碼。',400);
  ...
  }catch{return fail('source_unreachable','暫時無法連線 tz，尚未完成綁定，請稍後重試。',502);}
  finally{input.password='';}
  if(upstream.status===401)return fail('login_rejected','tz 未接受登入，請檢查帳密或帳號狀態。',422);
  if(upstream.status===403||upstream.status>=300&&upstream.status<400)return fail('source_access_denied','tz 拒絕這次連線，可能需要在官方網站完成驗證；目前未完成綁定。',502);
  if(upstream.status===429)return fail('source_rate_limited','tz 暫時限制登入次數，請稍後重試。',429);
  if(!upstream.ok)return fail('source_unavailable','tz 登入服務暫時無法使用，請稍後重試。',502);
  let result:any;try{result=await upstream.json();}catch{return fail('source_format_changed','tz 未回傳可辨識的登入結果，尚未完成綁定。',502);}
  if(String(result?.code)!=='200')return fail('login_rejected','tz 未接受登入，請檢查帳密，或到官方網站確認是否需要驗證。',422);
  const data=result.data;
  if(typeof data?.token!=='string'||!['number','string'].includes(typeof data?.user_id)||typeof data?.username!=='string'||data.username.toLowerCase()!==username.toLowerCase())return fail('source_format_changed','tz 登入回應不完整，尚未完成綁定。',502);
  let expiresAt:number;try{expiresAt=loginExpiry(data.token,now);}catch{return fail('source_invalid_token','tz 未提供有效期限內的授權，請重新登入。',502);}
  ...
  return reply({...summary(saved),configured:true,message:'tz 登入驗證成功，已保存個人授權。'});
 }catch{
