...
export const WIN_MODEL_VERSION='multifactor-trial-v1';
...
export function multifactorWin(g:Match,report:AnalysisReport|undefined,now:number){
 const p=baseProbability(g),factors:WinFactor[]=[];
 ...
 factors.push({name:'本季戰績',weight:20,home:wp('home'),away:wp('away'),score:p===null?null:2*p-1,contribution:p===null?0:20*(p-.5),rule:'雙方各加 10 勝 10 敗，以 log5 對戰機率轉為優勢分數'});
 add('先發本季 ERA',20,g.home.pitcherId?g.home.pitcherEra:null,g.away.pitcherId?g.away.pitcherEra:null,4,true,'ERA 差 ÷ 4；較低有利');
 add('先發本季 WHIP',10,g.home.pitcherId?g.home.pitcherWhip:null,g.away.pitcherId?g.away.pitcherWhip:null,.8,true,'WHIP 差 ÷ 0.8；較低有利');
 add('先發近期 ERA',10,f.home_starter_recent_era,f.away_starter_recent_era,4,true,'來源近期登板至少 3 場；ERA 差 ÷ 4；較低有利');
 ...
 add('九棒對左右投 wRC+',20,confirmed?f.home_lineup_wrc_plus:null,confirmed?f.away_lineup_wrc_plus:null,60,false,'確認九棒且各至少 30 打席；平均 wRC+ 差 ÷ 60；較高有利');
 add('牛棚近三日用球數',7,f.home_bullpen_last3_pitches,f.away_bullpen_last3_pitches,180,true,'用球數差 ÷ 180；用量較低有利（非疲勞實測）');
 add('牛棚連日登板人數',3,f.home_bullpen_back_to_back,f.away_bullpen_back_to_back,5,true,'人數差 ÷ 5；較少有利');
 for(const name of ['傷兵影響','天氣與球場','主場優勢']) factors.push({name,weight:name==='傷兵影響'?4:3,home:null,away:null,score:null,contribution:0,rule:name==='傷兵影響'?'缺球員替代價值，不以傷兵人數硬扣分':name==='天氣與球場'?'缺已驗證場地與雙方差異效應':'尚無本聯盟回測係數，暫不加成'});
