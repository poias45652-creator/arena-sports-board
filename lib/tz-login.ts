...
export async function tzLogin(request:Request,db:any,secret:string|undefined,fetcher:typeof fetch=fetch,turnstileSecret?:string,requireTurnstile=false){
 ...
 const username=input.username.trim();const now=Date.now();const throttle='login-rate:'+await hash(username.toLowerCase());
 ...
 const member='tz:'+String(binding.source_user_id),key=await credentialKey(secret);
 ...
 if(binding.username.toLowerCase()===PLATFORM_ADMIN_USERNAME)await db.prepare('INSERT INTO account_access (member_id,enabled,expires_at,updated_at) VALUES (?,1,NULL,?) ON CONFLICT(member_id) DO NOTHING').bind(member,Date.now()).run();
 ...
 return response({signedIn:true,username:binding.username},200,sessionCookie(session,Math.floor((expires-now)/1000)));
