...
async function sportsnaviNpbPhotos(code:string){const teamId=SPORTS_NAV_ID[code];if(!teamId)return new Map<string,string>();const html=(await getPage(`https://baseball.yahoo.co.jp/npb/teams/${teamId}/players`)).html,photos=new Map<string,string>();for(const row of html.matchAll(/<img\b[^>]*class=["']bb-playerList__img["'][^>]*>/gi)){const name=row[0].match(/alt=["']([^"']+)["']/)?.[1],src=row[0].match(/src=["']([^"']+)["']/)?.[1];if(name&&src&&src.startsWith('https://sports-baseball.west.edge.storage-yahoo.jp/'))photos.set(photoKey(plain(name)),src);}return photos;}
...
async function myKboPhotos(code:string,year:number){const slug=MYKBO_TEAM[code];if(!slug)return new Map<string,string>();const urls=[`https://mykbostats.com/teams/${slug}/${year}`,`https://mykbostats.com/teams/${slug}/roster`],settled=await Promise.allSettled(urls.map(url=>getPage(url))),photos=new Map<string,string>();for(const item of settled){if(item.status!=='fulfilled')continue;const page=item.value;for(const tr of page.html.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)){const link=tr[1].match(/href=["']\/players\/(\d+)(?:-[^"']*)?["'][^>]*>([\s\S]*?)<\/a>/i);if(!link)continue;const name=plain(link[2]),src=tr[1].match(/https?:\/\/mykbostats\.com\/photos\/player\/${link[1]}\/\d+\.jpg(?:\?[^"'\s<]*)?/i)?.[0];if(name&&src&&!/player_photo_missing/i.test(src))photos.set(photoKey(name),src);}}return photos;}
...
export async function collectProfilePlayers(league:ProfileLeague,code:string,year:number){
 ...
 if(league==='CPBL'){
  ...
  {const settled=await Promise.allSettled([npbPhotos(pages),sportsnaviNpbPhotos(code)]),official=settled[0].status==='fulfilled'?settled[0].value:new Map<string,string>(),media=settled[1].status==='fulfilled'?settled[1].value:new Map<string,string>(),map=new Map([...media,...official]);photoIndex=Object.fromEntries(map);photos=attachPhotos(result,map);if(official.size)sources.push({label:'NPB 官方球員照片',url:'https://npb.jp/bis/players/'});if(media.size)sources.push({label:'Sportsnavi 球員照片備援',url:`https://baseball.yahoo.co.jp/npb/teams/${SPORTS_NAV_ID[code]}/players`});if(settled.some(x=>x.status==='rejected'))warnings.push('部分球員照片來源暫時無法讀取');}
 }else{
  ...
  {const settled=await Promise.allSettled([kboPhotos(code,year),myKboPhotos(code,year)]),official=settled[0].status==='fulfilled'?settled[0].value:new Map<string,string>(),foreign=settled[1].status==='fulfilled'?settled[1].value:new Map<string,string>(),map=new Map([...foreign,...official]);photoIndex=Object.fromEntries(map);photos=attachPhotos(result,map);if(official.size)sources.push({label:'KBO 官方球員照片',url:KBO_SEARCH});if(foreign.size)sources.push({label:'MyKBO 球員照片備援',url:`https://mykbostats.com/teams/${MYKBO_TEAM[code]}/roster`});if(settled.some(x=>x.status==='rejected'))warnings.push('部分球員照片來源暫時無法讀取');}
 }
