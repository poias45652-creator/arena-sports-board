  ...
  const task=(async()=>{const result={...empty,rows:[],teamRuns:[],sources:[],errors:[]},deadline=AbortSignal.timeout(35000);
   ...
   if(league==='NPB'){
    ...
    if(scheduleOk){const byTeam=new Map(teams.map(team=>[team,[...rawGames.values()].filter(g=>[KBO[g.awayTeamCode],KBO[g.homeTeamCode]].includes(team)).sort((a,b)=>b.gameId.localeCompare(a.gameId)).slice(0,10)])),chosen=new Map([...byTeam.values()].flat().map(g=>[g.gameId,g]));
     const games=[...chosen.values()];for(let i=0;i<games.length;i+=6)await Promise.all(games.slice(i,i+6).map(async g=>{if(gameCache.has(g.gameId))return;try{if(!/^\d{8}[A-Z0-9]{4,20}$/.test(g.gameId))throw Error('場次 ID 不符');const p=await get(root+'/schedule/games/'+g.gameId+'/game-polling'),j=JSON.parse(p.text);if(j.success!==true||j.result?.game?.gameId!==g.gameId)throw Error('韓職日誌場次不符');const row=kboReliefGame(j.result.game,j.result.textRelayData,p,date);if(gameCache.size>350)gameCache.delete(gameCache.keys().next().value);gameCache.set(g.gameId,row);}catch(e){result.errors.push(g.gameId+'：'+e.message);}}));
     for(const [team,selected] of byTeam){const logs=selected.map(g=>gameCache.get(g.gameId));if(logs.length<5||logs.some(g=>!g)){result.errors.push(team+'：最近 10 場後援日誌尚未完整');continue;}const rows=logs.map(g=>g.teams.find(t=>t.team===team));if(rows.some(r=>!r))continue;const sum=k=>rows.reduce((s,r)=>s+r[k],0),outs=sum('outs');if(outs<=0)continue;
      const source={name:'Naver 韓職逐場後援紀錄彙算',url:logs[0].url,observedAt:new Date(now()).toISOString(),throughDate:logs[0].date,scope:'recent',games:logs.length,note:`最近 ${logs.length} 場已完賽紀錄，排除先發；先合計出局數與責失再算 ERA。`,gameIds:logs.map(g=>g.id)};result.rows.push({team,stats:stats(outs,sum('er'),sum('k'),'','',sum('bb'),sum('hits')),source});
     }
    }
   }
