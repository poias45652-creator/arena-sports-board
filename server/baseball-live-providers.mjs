...
export function tableData(html){return [...html.matchAll(/<table\b([^>]*)>([\s\S]*?)<\/table>/gi)].map(m=>({attrs:m[1],rows:[...m[2].matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)].map(r=>[...r[1].matchAll(/<t[hd]\b[^>]*>([\s\S]*?)<\/t[hd]>/gi)].map(c=>plain(c[1]))).filter(r=>r.length)}));}
...
export function parseKbo(raw,page,relay=null){
 ...
 g.rawStatus=raw.statusCode;g.status=raw.cancel?'cancelled':raw.suspended?'suspended':({BEFORE:'pregame',STARTED:'live',PLAYING:'live',IN_PROGRESS:'live',RESULT:'final',CANCEL:'cancelled'}[raw.statusCode]||'unknown');const played=['live','final','suspended'].includes(g.status);const state=relay?.currentGameState;
 for(const side of ['away','home']){
  ...
  const starter=(relay?.[side+'Lineup']?.pitcher||[]).find(p=>p.name===s&&Number(p.seqno)===1);
  if(starter&&g.starters[side])Object.assign(g.starters[side],{id:String(starter.pcode),era:numberOrNull(starter.seasonEra),source:provenance('naver',page)});
...
export function parseNpb(scoreHtml,statsHtml,id,page){
 ...
 const detail=classBlock(scoreHtml,'bb-gameDetail'),liveHtml=classBlock(scoreHtml,'bb-live')||detail;assert(liveHtml,'NPB live component missing');
 if(detail){const teams=[...detail.matchAll(/href="\/npb\/teams\/(\d+)\/index"/g)].map(m=>m[1]);assert(teams.length===2&&teams[0]===g.home?.id&&teams[1]===g.away?.id,'NPB detail team identity conflict');}
 const live=plain(liveHtml);g.rawStatus=live.slice(0,160);g.status=/試合終了|コールド/.test(live)?'final':/試合中止/.test(live)?'cancelled':/試合前|開始前/.test(live)||/見どころ/.test(live)&&!!classText(detail,'bb-gameCard__time').match(/^\d{1,2}:\d{2}$/)?'pregame':/中断/.test(live)?'suspended':/\d+回[表裏]/.test(live)?'live':'unknown';
 ...
 if(statsHtml){const statsTitle=plain(statsHtml.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1]||'');assert(statsTitle.startsWith(`${Number(m[1])}年${Number(m[2])}月${Number(m[3])}日`),'NPB stats date conflict');
...
export async function collectLeague(league,{date=dayInTaipei(),fetcher=fetch}={}){
 ...
 if(league==='KBO'){
  ...
  await Promise.all(found.slice(0,12).map(async row=>{const g=await attempt(errors,async()=>{assert(/^[A-Za-z0-9]+$/.test(row.gameId),'Invalid KBO ID');const p=await get(root+'/schedule/games/'+row.gameId+'/game-polling');const r=JSON.parse(p.text);assert(r.success===true&&r.result?.game?.gameId===row.gameId,'KBO detail identity conflict');return parseKbo(r.result.game,p,r.result.textRelayData);});if(g)games.push(g);}));
 }else if(league==='CPBL'){
  const candidates=new Map();await Promise.all(['富邦','統一','台鋼','中信','樂天','味全'].map(name=>attempt(errors,async()=>{const p=await get('https://tw.sports.yahoo.com/cpbl/teams/'+encodeURIComponent(name)+'/');for(const x of yahooObjects(p.text)){if(/^cpbl\.g\.\d{9}$/.test(x.gameId)&&x.seasonPhase==='REGULAR_SEASON'&&x.startTime&&CPBL[x.homeTeamId]&&CPBL[x.awayTeamId]&&dayInTaipei(new Date(x.startTime))===date){const url=x.alias?.url;if(typeof url==='string'&&/^https:\/\/tw\.sports\.yahoo\.com\/cpbl\/[^/]+-\d{9}\/$/.test(url))candidates.set(x.gameId,{url,raw:x,page:p});}}})));
  assert(candidates.size||errors.length===0,'CPBL schedule unavailable');await Promise.all([...candidates].map(async([id,{url,raw,page}])=>{
   const g=await attempt(errors,async()=>{const p=await get(url);const choices=yahooObjects(p.text).filter(x=>x.gameId===id&&x.playerStats&&CPBL[x.homeTeamId]&&CPBL[x.awayTeamId]);assert(choices.length,'CPBL full game payload missing');return parseCpbl(choices.at(-1),p);});
   if(g)games.push(g);
   // A future schedule is valid before the box-score payload is published.
   // Keep its verified fixture, without inventing a lineup, pitcher or stats.
   else if(raw.status==='PREGAME'){const scheduled=await attempt(errors,async()=>parseCpbl(raw,page));if(scheduled)games.push(scheduled);}
  }));
 }else{
