...
export function parseKbo(raw,page,relay=null){
 ...
 g.rawStatus=raw.statusCode;g.status=raw.cancel?'cancelled':raw.suspended?'suspended':({BEFORE:'pregame',READY:'pregame',STARTED:'live',PLAYING:'live',IN_PROGRESS:'live',RESULT:'final',CANCEL:'cancelled'}[raw.statusCode]||'unknown');const played=['live','final','suspended'].includes(g.status);const state=relay?.currentGameState;
 for(const side of ['away','home']){
