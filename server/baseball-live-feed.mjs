/** Request-driven, shared-source feed. No account credentials leave this server. */
export function createLiveFeed({collect,read,write,day,now=Date.now}) {
 const cache=new Map(),pending=new Map();
 return async function get(league) {
  if(!['NPB','KBO','CPBL'].includes(league))throw new Error('Invalid league');
  const date=day(),key=league+':'+date,previous=cache.get(key);
  if(previous&&previous.until>now())return previous.value;
  if(pending.has(key))return pending.get(key);
  for(const k of cache.keys())if(!k.endsWith(':'+date))cache.delete(k);
  const task=(async()=>{
   let stored=[];
   ...
   try{
    ...
    for(const g of fresh.games){
     const old=older.get(g.key);
     if(old&&(Date.parse(old.source.fetchedAt)>Date.parse(g.source.fetchedAt)||(old.status==='final'&&['pregame','unknown'].includes(g.status))))continue;
     games.set(g.key,{...g,sourceStale:false});
    }
    let persistence;
    try{persistence=await write(fresh);}catch{persistence={written:0,error:'本次資料尚未成功保存；其他裝置可能仍讀到先前資料。'};}
    const rows=[...games.values()],partial=errors.length>0||rows.some(g=>g.sourceStale);
    const pollAfterMs=rows.some(g=>g.status==='live')||partial?60000:300000;
    const value={schemaVersion:1,league,date,games:rows,status:partial?'partial':'ok',stale:false,checkedAt:new Date(now()).toISOString(),collectedAt:fresh.collectedAt,persistence,pollAfterMs,liveLatencyVerified:false,automaticBackgroundSync:false,...(partial?{error:'部分場次更新失敗；舊資料保留原擷取時間。'}:{})};
    cache.set(key,{value,until:now()+pollAfterMs});return value;
   }catch{
    const games=[...older.values()].map(g=>({...g,sourceStale:true}));
    const value={schemaVersion:1,league,date,games,status:games.length?'stale':'unavailable',stale:true,checkedAt:new Date(now()).toISOString(),error:'目前無法更新來源；不代表今日沒有比賽。',pollAfterMs:60000,liveLatencyVerified:false,automaticBackgroundSync:false};
