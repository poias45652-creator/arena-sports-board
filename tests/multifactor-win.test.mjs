...
const vite=await createServer({configFile:false,server:{middlewareMode:true}});after(()=>vite.close());
const {multifactorWin}=await vite.ssrLoadModule('/lib/multifactor-win.ts');
...
test('equal inputs are 50/50, full weight budget and explicit 10% missing',()=>{const m=multifactorWin(g,report(),now);assert.equal(m.homeWin,.5);assert.equal(m.coverage,90);assert.equal(m.factors.reduce((a,x)=>a+x.weight,0),100);assert.equal(m.ready,true)});
...
test('missing data is omitted, not filled or weight redistributed',()=>{const m=multifactorWin(g,undefined,now);assert.equal(m.coverage,50);assert.equal(m.ready,false);assert.equal(m.factors.find(x=>x.name==='九棒對左右投 wRC+').score,null)});
