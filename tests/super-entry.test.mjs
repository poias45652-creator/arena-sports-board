...
const {superEntry,superEntryUrl}=await import(moduleUrl('lib/super-entry.ts'));
...
test('rejects redirected or malicious destinations and malformed tickets',async()=>{for(const u of ['http://hr9988.net/#/APILogin?MemID='+ticket,'https://hr9988.net.evil.test/#/APILogin?MemID='+ticket,'https://evil.test/#/APILogin?MemID='+ticket,'https://user@hr9988.net/#/APILogin?MemID='+ticket,'https://hr9988.net/?redirect=evil#/APILogin?MemID='+ticket,'https://hr9988.net/#/APILogin?MemID=bad','https://hr9988.net/#/APILogin?MemID='+ticket+'&MemID='+ticket])assert.equal(superEntryUrl(u),null);const {db}=await setup();const r=await superEntry(request(),'a',db,secret,async()=>valid('https://evil.test/'));assert.equal(r.status,502);assert.equal((await r.text()).includes('evil.test'),false);});
test('source denial fails closed without replay, member rate limits stay isolated',async()=>{const {db}=await setup();let calls=0;const denied=async()=>{calls++;return new Response('sensitive upstream content',{status:403});};const r=await superEntry(request(),'a',db,secret,denied);assert.equal(r.status,502);assert.equal((await r.text()).includes('sensitive'),false);assert.equal((await superEntry(request(),'a',db,secret,denied)).status,429);assert.equal(calls,1);assert.equal((await superEntry(request(),'b',db,secret,async()=>valid())).status,200);});
test('changed account authorization cannot return an old ticket; mobile entry uses source device flag',async()=>{const {db,sql}=await setup();const r=await superEntry(request('https://arena.test',{'user-agent':'iPhone Mobile'}),'a',db,secret,async(u,o)=>{assert.equal(JSON.parse(o.body).game_device,'Mobile');sql.prepare('UPDATE tz_bindings SET verified_at=2 WHERE member_id=?').run('a');return valid();});assert.equal(r.status,409);assert.equal((await r.text()).includes(ticket),false);});
test('oversized source response is rejected rather than reflected',async()=>{const {db}=await setup();const r=await superEntry(request(),'a',db,secret,async()=>new Response('x'.repeat(70000)));assert.equal(r.status,502);assert.ok((await r.text()).length<300);});
