...
import pg from 'pg';
import {DATABASE_SCHEMA} from '../server/database.mjs';
...
if(!result?.ok||!result.refresh?.enabled||lanes.length!==3||lanes.some(x=>!x.succeededAt)){
 const startup=await readFile('/tmp/yj-runtime-smoke.log','utf8').catch(()=>'');
 console.log('CI_STARTUP_DIAGNOSTICS',startup.split('\n').filter(line=>/baseball-background|Error|Failed|Unable|Ready/.test(line)).slice(-12).join('\n'));
 throw Error('Three runtime refresh loops did not complete a successful check');
}
const pool=new pg.Pool({connectionString:process.env.DATABASE_URL,options:`-c search_path=${DATABASE_SCHEMA},pg_catalog`});
