...
for (let attempt = 1; attempt <= 15; attempt++) {
  try {
    ...
    assert.equal(health.database, true); assert.equal(health.adminSetupVersion, 2);
    const metaResponse = await fetch(`${base}/api/meta`, { cache: 'no-store', signal: AbortSignal.timeout(10000) });
    assert.equal(metaResponse.status, 200);
    const meta = await metaResponse.json();
    assert.equal(meta.adminSetupVersion, 2); assert.equal(typeof meta.setupRequired, 'boolean');
    console.log(JSON.stringify({ readOnly: true, health: 'passed', database: true, adminSetupVersion: 2, setupRequired: meta.setupRequired }));
    verified = true; break;
